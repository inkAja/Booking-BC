                ...
                <div id="table-default" class="table-responsive">
                  <table class="table">
                    ...
                    <tbody class="table-tbody">
                      <?php 
                      $datas = mysqli_query($connect, "SELECT * FROM categories ORDER BY name ASC");
                      $no = 1;

                      while($category = mysqli_fetch_array($datas)){
                    
                      ?>
                        <td class="sort-no"><?php echo $no++ ?></td>
                        <td class="sort-name"><?php echo $category["name"]?></td>
                        <td class="sort-action"><td>
                            <div class="btn-list flex-nowrap">
                              <a href="<?php echo $base_url;?>/category/edit.php?id=<?php echo $category["id"]?>" class="btn btn-info">
                                Edit
                              </a>
                              
                                  <a href="<?php echo $base_url;?>/category/delete.php?id=<?php echo $category["id"]?>" class="btn btn-danger">
                                    Delete
                                  </a>
